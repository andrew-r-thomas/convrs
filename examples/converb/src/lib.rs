...
use realfft::{num_traits::real::Real, RealFftPlanner};
use std::sync::Arc;
...
impl Plugin for Converb {
    ...
    fn initialize(
        ...
        let mut reader = hound::WavReader::open("IRs/shortsweet.wav").unwrap();
        let samples = reader.samples::<f32>();
        let mut vec: Vec<f32> = samples.collect::<Vec<f32>>();
        let planner = RealFftPlanner::new();
        let fft = planner.plan_fft_forward(vec.len());
        let mut out = fft.make_output_vec();
        fft.process(&mut vec, &mut out).unwrap();
