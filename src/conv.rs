...
struct SegmentHandle {
    ...
    filter_chunk_prod: Producer<f32>,
...
impl Conv {
    pub fn new(partition: &[(usize, usize)], channels: usize) -> Self {
        ...
        if partition.len() > 1 {
            ...
            for i in 1..partition.len() {
                ...
                let (filter_chunk_prod, mut filter_chunk_cons) =
                    RingBuffer::<f32>::new(p.0 * channels * 1000);
                ...
                thread::spawn(move || {
                    ...
                    loop {
                        ...
                        if !filter_chunk_cons.is_empty() {
                            match filter_chunk_cons.read_chunk(p.0 * channels) {
                                Ok(r) => {
                                    let (s1, s2) = r.as_slices();
    ...
    pub fn push_filter_block<'block>(
        &mut self,
        filter_blocks: impl Iterator<Item = &'block [f32]>,
    ) {
        self.filter_cycle_count += 1;

        for (filter_channel, filter_block) in self
            .filter_buff
            .chunks_exact_mut(self.buff_len)
            .zip(filter_blocks)
        {
            filter_channel.copy_within(self.block_size..self.buff_len, 0);
            filter_channel[self.buff_len - self.block_size..self.buff_len]
                .copy_from_slice(filter_block);
        }
            ...
            if self.filter_cycle_count % segment.avail == 0 {
                    ...
                    .write_chunk(segment.block_size * self.channels)
                {
                    Ok(mut w) => {
                        ...
                        for in_channel in self.filter_buff.chunks_exact(self.buff_len) {
                            let to_write =
